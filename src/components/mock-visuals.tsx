...
export function MiniAd({ ratio = '4/5', headline, sub, cta = 'Learn more', brand = 'BRAND', label, className = '' }: MiniAdProps) {
  return (
    <div className={`relative overflow-hidden rounded-[10px] border border-line bg-paper ${className}`} style={{ aspectRatio: ratio }}>
      {/* Photo placeholder */}
      <div className="absolute inset-0" style={{ background: 'repeating-linear-gradient(135deg, transparent 0 14px, oklch(0 0 0 / 0.04) 14px 15px), var(--color-paper-2)' }} />
      {/* Chrome bar */}
      ...
      {label && (
        <div className="absolute top-2.5 right-3 z-10 font-mono text-[9px] tracking-widest uppercase bg-ink text-paper px-[7px] py-[3px] rounded">
          {label}
        </div>
      )}
      {headline && (
      ...
      <div className="absolute left-3 right-3 bottom-3 bg-[oklch(0_0_0/0.05)] rounded-md px-2.5 py-[7px] flex justify-between items-center font-mono text-[9px] text-ink tracking-wider">
        <span>{cta}</span>
...
export function MiniLP({ headline = 'A page that explains the offer.', sub = 'Built for clarity, not decoration.', cta = 'Get started', className = '' }: MiniLPProps) {
  return (
    <div className={`bg-paper border border-line rounded-xl overflow-hidden ${className}`}>
      {/* Browser chrome */}
      <div className="flex gap-1.5 px-3.5 py-2.5 border-b border-line bg-paper-2">
      ...
      <div className="p-6">
        ...
        <div className="inline-block px-3.5 py-2 rounded-full bg-ink text-paper text-[11px] font-medium">{cta} &rarr;</div>
        <div className="grid grid-cols-3 gap-2 mt-5">
          <div className="h-[52px] rounded-md bg-paper-2 border border-line" />
          <div className="h-[52px] rounded-md bg-paper-2 border border-line" />
          <div className="h-[52px] rounded-md bg-paper-2 border border-line" />
        </div>
      </div>
